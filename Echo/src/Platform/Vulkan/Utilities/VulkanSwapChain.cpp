...
namespace Echo 
{
	...
	VulkanSwapChain::VulkanSwapChain(Extent2D extent)
		: m_Device((VulkanDevice*) Application::Get().GetWindow().GetDevice()), m_WindowExtent(VkExtent2D{extent.width, extent.height})
	{
		Init();
	}

	VulkanSwapChain::VulkanSwapChain(Extent2D extent, Ref<VulkanSwapChain> previous)
		: m_Device((VulkanDevice*) Application::Get().GetWindow().GetDevice()), m_WindowExtent(VkExtent2D{ extent.width, extent.height }), m_OldSwapChain(previous)
	{
		Init();
	...
	void VulkanSwapChain::CreateImageViews()
	{
		...
		for (size_t i = 0; i < m_SwapChainImages.size(); i++)
		{
			...
			if (vkCreateImageView(m_Device->GetDevice(), &viewInfo, nullptr, &m_SwapChainImageViews[i]) !=
				VK_SUCCESS)
			{
				EC_CORE_CRITICAL("Failed to create texture image view!")
			}
		}
	}
	...
	void VulkanSwapChain::CreateFramebuffers()
	{
		...
		for (size_t i = 0; i < GetImageCount(); i++)
		{
			...
			{
				EC_CORE_CRITICAL("Failed to create framebuffer!")
			}
		}
	}
	...
	void VulkanSwapChain::CreateSyncObjects()
	{
		...
		for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
		{
			...
			{
				EC_CORE_CRITICAL("Failed to create synchronization objects for a frame!")
			}
		}
	}
		...
		const std::vector<VkPresentModeKHR>& availablePresentModes)
	{
		for (const auto& availablePresentMode : availablePresentModes)
		{
			...
			{
				EC_CORE_INFO("Present mode: Mailbox")
		...
		EC_CORE_INFO("Present mode: V-Sync")

...
namespace Echo 
{
	...
	void VulkanPipeline::Start() 
	{
		VkDevice device = m_Context->GetVulkanDevice()->GetDevice();

		VkAttachmentDescription colorAttachment{};
		colorAttachment.format = m_Context->GetVulkanSwapChain()->GetSwapChainImageFormat();
	...
	VK_DYNAMIC_STATE_SCISSOR
		...
		VkExtent2D extent = m_Context->GetVulkanSwapChain()->GetSwapChainExtent();
		VkViewport viewport{};
		viewport.x = 0.0f;
		viewport.y = 0.0f;
	...
	VkShaderModule VulkanPipeline::CreateShaderModule(const std::vector<char>& code)
	{
		...
		if (vkCreateShaderModule(m_Context->GetVulkanDevice()->GetDevice(), &createInfo, nullptr, &shaderModule) != VK_SUCCESS)
		{
